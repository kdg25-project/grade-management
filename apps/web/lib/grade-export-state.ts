import type { GradeExportFormat, GradeExportQuery, GradeExportScope } from "./admin-api";
...
export const gradeExportQuery = (values: { year: string; scope: GradeExportScope; format: GradeExportFormat; term: string; courseId: string; gradeLevel: string; subjectId: string }): GradeExportQuery => ({ academicYear: /^\d{4}$/.test(values.year) ? Number(values.year) : undefined, scope: values.scope, format: values.format, term: values.scope === "term" && (values.term === "1" || values.term === "2") ? Number(values.term) as 1 | 2 : undefined, courseId: values.courseId || undefined, gradeLevel: ["1", "2", "3"].includes(values.gradeLevel) ? Number(values.gradeLevel) as 1 | 2 | 3 : undefined, subjectId: values.subjectId || undefined });
