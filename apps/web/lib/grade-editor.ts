...
export type DraftRow = { studentId: string; studentNumber: string; name: string; status: GradeStudent["status"]; editable: boolean; hasFailedHistory: boolean; values: DraftValues; score: DraftScore | null };
export type GradeField = keyof DraftValues;

export const fieldRange: Record<GradeField, readonly [number, number]> = { attendanceRate: [0, 100], attitude: [0, 10], assignment: [0, 10] };
...
export function rowsFromStudents(students: GradeStudent[]): DraftRow[] {
  return students.map((student) => ({ studentId: student.id, studentNumber: student.studentNumber, name: student.name, status: student.status, editable: student.editable, hasFailedHistory: student.hasFailedHistory, values: { attendanceRate: student.grade?.attendanceRate == null ? "" : String(student.grade.attendanceRate), attitude: student.grade?.attitude == null ? "" : String(student.grade.attitude), assignment: student.grade?.assignment == null ? "" : String(student.grade.assignment) }, score: student.grade }));
}
...
export function draftHasErrors(rows: DraftRow[]) {
  return rows.some((row) => (Object.keys(row.values) as GradeField[]).some((field) => validateDraftValue(field, row.values[field]) !== null));
}
...
export function draftPayload(rows: DraftRow[]) {
  return rows.filter((row) => row.editable && row.status === "enrolled").map((row) => ({ studentId: row.studentId, attendanceRate: row.values.attendanceRate === "" ? null : Number(row.values.attendanceRate), attitude: row.values.attitude === "" ? null : Number(row.values.attitude), assignment: row.values.assignment === "" ? null : Number(row.values.assignment) }));
}
