...
export function AdminGradeSearchPage() {
  ...
  return <TeacherShell variant="admin" navigationGuard={guard}>
    ...
    {detail && latest ? <section className="masterCard"><h2>{detail.studentName}さん・{detail.subjectName}</h2><p>成績履歴（古い成績は変更できません）</p><ul className="simpleList">{detail.attempts.map((attempt) => <li key={attempt.id}>{attempt.hasFailedHistory ? <span title="この科目・学期に確定済みの不可評価履歴があります">▲ </span> : null}{attempt.attempt}回目: {attempt.letterGrade ? displayGrade(attempt.letterGrade) : "未入力"}</li>)}</ul><div className="masterForm"><label>出席率（0〜100）<input inputMode="numeric" disabled={controlsDisabled} value={draft.attendanceRate} onChange={(event) => update("attendanceRate", event.target.value)} /></label><label>授業態度（1〜10）<input inputMode="numeric" disabled={controlsDisabled} value={draft.attitude} onChange={(event) => update("attitude", event.target.value)} /></label><label>課題（1〜10）<input inputMode="numeric" disabled={controlsDisabled} value={draft.assignment} onChange={(event) => update("assignment", event.target.value)} /></label><label>理由<textarea disabled={controlsDisabled} value={draft.reason} onChange={(event) => update("reason", event.target.value)} /></label></div><div className="editorActions"><button className="primaryAction compactAction" disabled={controlsDisabled || !dirty} type="button" onClick={() => void save("correction")}>{saving ? "保存中…" : "最新成績を修正"}</button><button className="secondaryAction compactAction" disabled={controlsDisabled || latest.letterGrade !== "F" || !dirty} type="button" onClick={() => void save("retake")}>再試験を登録</button></div></section> : null}
  </TeacherShell>;
}
