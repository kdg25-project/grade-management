...
export function AdminGradeSearchPage() {
  ...
  async function save(kind: "correction" | "retake") {
    ...
    const payload = adminGradePayload(draft); if (!payload) { setError("出席率は0〜100、平常点・課題点は1〜10の整数で、理由を入力してください。"); return; }
    const latest = detail.attempts[0]; if (!latest) return;
    if (kind === "retake" && latest.letterGrade !== "F") { setError("最新評価がFの場合のみ再試験を登録できます。"); return; }
    if (!window.confirm(kind === "retake" ? "再試験の成績を登録しますか？" : "最新の成績を修正しますか？")) return;
    ...
    } catch (nextError) { setError(nextError instanceof GradeApiError ? nextError.message : "成績を保存できませんでした。"); }
