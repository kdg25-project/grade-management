...
export function GradeDetail() {
  ...
  const [initialWeightPrompt, setInitialWeightPrompt] = useState(false);
  const savingRef = useRef(false);
  const requestGeneration = useRef(0);
  const controller = useRef<AbortController | null>(null);
  const weightDialog = useRef<HTMLDialogElement | null>(null);
  ...
  const load = useCallback(async ({ preserveData = false }: { preserveData?: boolean } = {}) => {
    ...
    try {
      ...
      setInitialWeightPrompt(needsInitialWeightSave(next.weights, next.editable));
      return true;
    } catch (nextError) {
  ...
  useEffect(() => {
    const dialog = weightDialog.current;
    if (!dialog) return;
    if (weightDialogOpen && !dialog.open) dialog.showModal();
    else if (!weightDialogOpen && dialog.open) dialog.close();
  }, [weightDialogOpen]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (initialWeightPrompt && editable && loadedContext.current === contextKey) setWeightDialogOpen(true);
  }, [contextKey, editable, initialWeightPrompt]);
  useEffect(() => {
    if (!editable && weightDialogOpen) setWeightDialogOpen(false);
  }, [editable, weightDialogOpen]);
  ...
  const openWeightDialog = () => {
    ...
    if (!weightDraft) setWeightDraft(data?.weights ?? defaultWeights);
    setWeightState("idle");
    setNotice(null);
    setWeightDialogOpen(true);
  };
  const dismissWeightDialog = () => {
    if (isSaving) return;
    setWeightDraft(data?.weights ?? defaultWeights);
    setWeightsDirty(needsInitialWeightSave(data?.weights ?? null, Boolean(data?.editable)));
    setInitialWeightPrompt(false);
    setWeightState("idle");
    setNotice(null);
    setWeightDialogOpen(false);
  ...
  return <TeacherShell navigationGuard={confirmDiscard}><Link className="backLink" to={year ? `/teacher/subjects?year=${year}` : "/teacher/subjects"} onClick={(event) => { if (!confirmDiscard()) event.preventDefault(); }}><ChevronLeft aria-hidden="true" />担当科目に戻る</Link><header className="pageHeader gradeHeader"><p className="sectionEyebrow">{data?.academicYear ?? year ?? ""}年度 {termLabel(term)}</p><h1>成績表</h1><p>入力対象の学期のみ編集できます。確定済みの成績は閲覧専用です。</p></header>
    ...
    {notice && weightState !== "failed" ? <p className={saveState === "failed" ? "formError" : "formSuccess"} role={saveState === "failed" ? "alert" : "status"}>{notice}</p> : null}
    {!error && data && loadedContext.current === contextKey ? <><section className="entrySummary" aria-label="入力状況"><div><p className="sectionEyebrow">入力状況</p><p className="summaryNumber"><strong>{displayRows.filter((row) => row.status === "enrolled" && row.values.attendanceRate !== "" && row.values.attitude !== "" && row.values.assignment !== "").length}</strong> / {displayRows.filter((row) => row.status === "enrolled").length} 名の入力がそろっています</p><p className="mutedText">{data.isFinalized ? "確定済みのため、講師は編集できません。" : data.editable ? "変更は保存ボタンで反映されます。" : "過去年度は閲覧専用です。"}</p></div><div className="saveStatus" aria-live="polite"><span className={dirty ? "statusIcon changed" : "statusIcon"} aria-hidden="true" /><div><strong>{dirty ? "未保存の変更があります" : saveState === "saved" ? "保存しました" : "変更はありません"}</strong><p>{saveState === "saving" ? "保存中…" : saveState === "failed" ? "保存できませんでした" : ""}</p></div></div></section>
      <section className="weightPanel" aria-labelledby="weight-heading"><div><p className="sectionEyebrow">評価比重</p><h2 id="weight-heading">成績の計算方法</h2><p>合計100になるように設定します。現在の入力学期のみ変更できます。</p></div><div className="weightPanelActions">{data.weights ? <dl className="weightSummary"><div><dt>出席率</dt><dd>{data.weights.attendanceWeight}%</dd></div><div><dt>授業態度</dt><dd>{data.weights.attitudeWeight}%</dd></div><div><dt>課題</dt><dd>{data.weights.assignmentWeight}%</dd></div></dl> : <p className="inputHint">初期の評価比重を設定してください。</p>}<button className="secondaryAction compactAction" type="button" aria-label="比重設定" disabled={!editable || isSaving} onClick={openWeightDialog}>設定</button></div></section>
      <dialog ref={weightDialog} className="weightDialog" aria-labelledby="weight-dialog-heading" onCancel={(event) => { event.preventDefault(); dismissWeightDialog(); }} onClick={(event) => { if (event.target === event.currentTarget) dismissWeightDialog(); }}><section><div className="weightDialogHeader"><div><p className="sectionEyebrow">評価比重</p><h2 id="weight-dialog-heading">成績の計算方法を設定</h2><p>合計100になるように設定してください。</p></div><button className="dialogClose" type="button" aria-label="比重設定を閉じる" disabled={isSaving} onClick={dismissWeightDialog}>×</button></div>{weightDraft ? <div className="weightControls">{(["attendanceWeight", "attitudeWeight", "assignmentWeight"] as const).map((field, index) => <label key={field}>{field === "attendanceWeight" ? "出席率" : field === "attitudeWeight" ? "授業態度" : "課題"}<div><input autoFocus={index === 0} type="number" min="0" max="100" step="1" value={weightDraft[field]} disabled={!editable || isSaving} onChange={(event) => updateWeight(field, Number(event.target.value))} /><span>%</span></div></label>)} </div> : null}{weightState === "failed" && notice ? <p className="formError" role="alert">{notice}</p> : null}{weightsNeedSaving ? <p className="inputHint">成績を入力する前に、初期の評価比重を保存してください。</p> : gradesDirty ? <p className="inputHint">先に成績を保存してから、比重を保存してください。</p> : null}<div className="weightDialogActions"><button className="secondaryAction compactAction" type="button" disabled={isSaving} onClick={dismissWeightDialog}>キャンセル</button><button className="primaryAction compactAction" type="button" disabled={!editable || isSaving || !canSaveWeights(gradesDirty, weightsDirty, isSaving)} onClick={() => void saveWeights()}>{weightState === "saving" ? "保存中…" : weightsNeedSaving ? "初期比重を保存" : "比重を保存"}</button></div></section></dialog>
