...
import { canApplyDraftChange, canEditInViewport, canSaveGrades, canSaveWeights, draftHasErrors, draftPayload, fieldLabel, formatScore, needsInitialWeightSave, previewRows, rowsFromStudents, type DraftRow, type GradeField, validateDraftValue, validateWeights } from "@/lib/grade-editor";
...
export function GradeDetail() {
  ...
  const invalid = draftHasErrors(rows);
  ...
  const updateValue = (studentId: string, field: GradeField, value: string) => { if (weightsNeedSaving || !canApplyDraftChange(isSaving || savingRef.current)) return; setRows((current) => current.map((row) => row.studentId === studentId ? { ...row, values: { ...row.values, [field]: value } } : row)); setGradesDirty(true); setSaveState("idle"); };
  ...
  return <TeacherShell navigationGuard={confirmDiscard}><Link className="backLink" to={year ? `/teacher/subjects?year=${year}` : "/teacher/subjects"} onClick={(event) => { if (!confirmDiscard()) event.preventDefault(); }}><ChevronLeft aria-hidden="true" />担当科目に戻る</Link><header className="pageHeader gradeHeader"><p className="sectionEyebrow">{data?.academicYear ?? year ?? ""}年度 {termLabel(term)}</p><h1>成績表</h1><p>入力対象の学期のみ編集できます。確定済みの成績は閲覧専用です。</p></header>
    ...
    {notice ? <p className={saveState === "failed" || weightState === "failed" ? "formError" : "formSuccess"} role="status">{notice}</p> : null}
    {!loading && !error && data ? <><section className="entrySummary" aria-label="入力状況"><div><p className="sectionEyebrow">入力状況</p><p className="summaryNumber"><strong>{displayRows.filter((row) => row.status === "enrolled" && row.values.attendanceRate !== "" && row.values.attitude !== "" && row.values.assignment !== "").length}</strong> / {displayRows.filter((row) => row.status === "enrolled").length} 名の入力がそろっています</p><p className="mutedText">{data.isFinalized ? "確定済みのため、講師は編集できません。" : data.editable ? "変更は保存ボタンで反映されます。" : "過去年度は閲覧専用です。"}</p></div><div className="saveStatus" aria-live="polite"><span className={dirty ? "statusIcon changed" : "statusIcon"} aria-hidden="true" /><div><strong>{dirty ? "未保存の変更があります" : saveState === "saved" ? "保存しました" : "変更はありません"}</strong><p>{saveState === "saving" ? "保存中…" : saveState === "failed" ? "保存できませんでした" : ""}</p></div></div></section>
      <section className="weightPanel" aria-labelledby="weight-heading"><div><p className="sectionEyebrow">評価比重</p><h2 id="weight-heading">成績の計算方法</h2><p>合計100になるように設定します。現在の入力学期のみ変更できます。</p></div>{weightDraft ? <div className="weightControls">{(["attendanceWeight", "attitudeWeight", "assignmentWeight"] as const).map((field) => <label key={field}>{field === "attendanceWeight" ? "出席率" : field === "attitudeWeight" ? "平常点" : "課題点"}<input type="number" min="0" max="100" step="1" value={weightDraft[field]} disabled={!editable || isSaving} onChange={(event) => updateWeight(field, Number(event.target.value))} /><span>%</span></label>)}<button className="secondaryAction compactAction" type="button" disabled={!editable || isSaving || !canSaveWeights(gradesDirty, weightsDirty, isSaving)} onClick={() => void saveWeights()}>{weightState === "saving" ? "保存中…" : weightsNeedSaving ? "初期比重を保存" : "比重を保存"}</button>{weightsNeedSaving ? <p className="inputHint">成績を入力する前に、初期の評価比重を保存してください。</p> : gradesDirty ? <p className="inputHint">先に成績を保存してから、比重を保存してください。</p> : null}</div> : null}</section>
      <div className="desktopGradeEditor"><table className="gradeTable"><caption>学生ごとの成績入力表</caption><thead><tr><th scope="col">学生</th><th scope="col">出席率<br /><span>0〜100</span></th><th scope="col">平常点<br /><span>0〜10</span></th><th scope="col">課題点<br /><span>0〜10</span></th><th scope="col">点数</th><th scope="col">評価</th></tr></thead><tbody>{displayRows.map((row) => <GradeRow key={row.studentId} row={row} editable={gradeEditable} isSaving={isSaving} updateValue={updateValue} />)}</tbody></table></div><div className="mobileGradeViewer" aria-label="成績表の閲覧専用表示">{displayRows.map((row) => <article className={row.status !== "enrolled" ? "gradeCard leaveCard" : "gradeCard"} key={row.studentId}><div><strong>{row.hasFailedHistory ? <span title="この科目・学期には確定済みのF評価履歴があります">▲ </span> : null}{row.name}</strong><span>{row.studentNumber}</span></div><p>{row.status !== "enrolled" ? "在籍対象外・入力不可" : "閲覧専用"}</p><dl><div><dt>出席率</dt><dd>{row.values.attendanceRate || "未入力"}</dd></div><div><dt>平常点</dt><dd>{row.values.attitude || "未入力"}</dd></div><div><dt>課題点</dt><dd>{row.values.assignment || "未入力"}</dd></div><div><dt>点数・評価</dt><dd>{formatScore(row.score?.finalScoreNumerator, row.score?.finalScoreDenominator)} / {row.score?.letterGrade ?? "未算出"}</dd></div></dl></article>)}</div><div className="editorActions"><button className="primaryAction compactAction" type="button" disabled={!gradeEditable || invalid || isSaving || !canSaveGrades(gradesDirty, weightsDirty, isSaving, !weightsNeedSaving)} onClick={() => void save()}><Save aria-hidden="true" />{saveState === "saving" ? "保存中…" : "変更を保存"}</button>{weightsNeedSaving || weightsDirty ? <p className="inputHint">先に評価比重を保存してから、成績を入力・保存してください。</p> : invalid ? <p className="inputHint">入力エラーを修正してから保存してください。</p> : null}</div></> : null}
  </TeacherShell>;
}
...
function GradeRow({ row, editable, isSaving, updateValue }: Readonly<{ row: DraftRow; editable: boolean; isSaving: boolean; updateValue: (studentId: string, field: GradeField, value: string) => void }>) {
  return <tr className={row.status !== "enrolled" ? "leaveRow" : undefined}><th scope="row"><strong>{row.hasFailedHistory ? <span title="この科目・学期には確定済みのF評価履歴があります">▲ </span> : null}{row.name}</strong><span>{row.studentNumber}</span>{row.status !== "enrolled" ? <em>入力対象外</em> : null}</th>{(Object.keys(fieldLabel) as GradeField[]).map((field) => { const value = row.values[field]; const message = validateDraftValue(field, value); const errorId = `grade-${row.studentId}-${field}-error`; return <td key={field}>{editable ? <><input aria-label={`${row.name}の${fieldLabel[field]}`} aria-describedby={message ? errorId : undefined} aria-invalid={Boolean(message)} className={message ? "invalidInput" : undefined} disabled={isSaving || !row.editable || row.status !== "enrolled"} inputMode="numeric" value={value} onChange={(event) => updateValue(row.studentId, field, event.target.value)} />{message ? <span className="inputHint" id={errorId}>{message}</span> : null}</> : <output className="readOnlyCell">{value || "未入力"}</output>}</td>; })}<td><output className="calculationPending">{formatScore(row.score?.finalScoreNumerator, row.score?.finalScoreDenominator)}</output></td><td><output className="calculationPending">{row.score?.letterGrade ?? "未算出"}</output></td></tr>;
}
