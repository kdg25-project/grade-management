...
const valid = {
  ...
  workflows: [{ binding: "DAILY_BACKUP_WORKFLOW", class_name: "DailyBackupWorkflow", schedules: ["0 17 * * *"] }],
};
...
describe("production deploy configuration", () => {
  ...
  it("rejects each unsafe production setting without echoing its value", () => {
    const invalid = { ...valid, vars: { ...valid.vars, BETTER_AUTH_URL: "http://localhost:5173", BETTER_AUTH_TRUSTED_ORIGINS: "https://grades.example.jp,http://localhost:5173", EMAIL_FROM: "no-reply@example.invalid", EMAIL_DELIVERY_ENABLED: "false", CLOUDFLARE_ACCOUNT_ID: "replace-with-account", D1_DATABASE_ID: "replace-with-database" }, d1_databases: [{ database_id: "00000000-0000-0000-0000-000000000000" }], send_email: [{ allowed_sender_addresses: ["another@example.jp"] }], r2_buckets: [{ binding: "BACKUP_BUCKET", bucket_name: "replace-with-bucket" }], workflows: [] };
    ...
    expect(errors).toContain("Daily backup Workflow schedule must be configured");
